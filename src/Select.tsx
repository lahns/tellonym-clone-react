...
const SelectSegment = <T extends string>({
  segmentValue,
  currentValue,
  onclick,
  type,
  children,
}: SelectSegmentProps<T> & PropsWithChildren) => {
  const selectedColors =
    "text-white border-primary-bg bg-primary-bg hover:border-primary-onBg hover:bg-primary-onBg";
  const unselectedColors =
    "text-gray-onBg border-gray-outline hover:bg-gray-outline hover:border-gray-outline";

  const chosenColors =
    segmentValue === currentValue ? selectedColors : unselectedColors;
...
const Select = <U extends Record<string, string>>({
  value,
  setValues,
  options,
}: SelectProps<U>) => {
  ...
  return <div className="flex cursor-pointer">{body()}</div>;
};
