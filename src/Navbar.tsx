      ...
      <div className="w-full lg:w-3/4 xl:w-2/3 justify-between items-center flex flex-row">
        <div className="w-1/7 md:w-1/5 lg:w-1/4 flex flex-row justify-start items-center">
          ...
          >
            ...
            <h1 className="md:block hidden text-gray-onBg text-2xl font-logo">
              Asklet
            </h1>
          </Link>
        </div>
        ...
        <div className="w-1/7 md:w-1/5 lg:w-1/4 flex flex-row justify-end items-center">
          {context.currentUser ? (
            ...
            <Link to={`/login`}>
              <Button.Primary additionalStyle="">Log in</Button.Primary>
            </Link>
          )}
        </div>
      </div>
...
const SearchBar = ({ userId, context, setContext }: { userId: number | undefined } & ReturnType<typeof useAppContext>) => {
  ...
  const filteredUsers = useMemo(
    () => users.filter(({ username }) => username.includes(query)),
    [users, query]
  );
  ...
  return (
    <div
      onBlur={closeSearchBlur}
      className="w-4/6 md:w-1/2 mx-2 relative justify-center items-center"
    >
      ...
      <input
        ...
        className="w-full p-1 bg-gray-bg focus:outline-none focus:border-primary-bg border-gray-outline border-2 rounded-lg"
      ></input>
      {isSearching && (
...
const ProfileResult = ({
  ...
  return (
    <Link
      ...
      to={`/user/${user.id}`}
      className="w-full flex flex-row items-center justify-between gap-2 px-2 hover:bg-gray-bg p-2"
    >
      <img
        alt=""
  ...
  return (
    ...
    >
      ...
      >
        <img
          alt=""
          src={`${config.ServerURL}/pfps/${userId}.png`}
          className="text-text-header rounded-full h-10 w-10"
        />
        <div className="md:block hidden text-text-header">
        ...
        <div className="rounded-lg overflow-hidden flex flex-col absolute bg-white drop-shadow-md justify-center top-full mt-2 right-0 md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-[175px] w-auto divide-y divide-gray-outline">
          <div className="flex flex-col">
            <Link
              ...
              className="inline-block p-4 hover:bg-gray-bg"
              to={`/user/${userId}`}
            >
              Profile
            </Link>
          ...
          <button
            className="inline-block text-left p-4 text-error-onBg hover:text-white hover:bg-error-bg"
            onClick={logOut}
          >
            Log out
          </button>
