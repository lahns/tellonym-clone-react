...
const Button = ({onClick, bgColor, textColor, border, additionalStyle, disabled, children}: ButtonProps & PropsWithChildren) => {
    ...
    :   
        <div onClick={onClick} className={`${additionalStyle} cursor-pointer w-fit h-fit p-2 px-4 ${border} ${bgColor} ${textColor} font-semibold rounded-lg`}>
            {children}
        </div>
}
...
Button.Primary = ({onClick, additionalStyle, disabled, children}: SubButtonProps) => 
    <Button
        disabled={disabled}
        onClick={onClick}
...
Button.Secondary = ({onClick, additionalStyle, disabled, children}: SubButtonProps) => 
    <Button
...
Button.Cancel = ({onClick, additionalStyle, disabled, children}: SubButtonProps) =>
    <Button
