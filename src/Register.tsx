      ...
      >
        {({errors, isSubmitting, handleSubmit}) =>
          <Form className='w-full flex flex-col p-3 justify-center items-center gap-2'>
                ...
                <label htmlFor="repeated_password" className='text-xl'>Confirm password:</label>
                <Field 
                  validate={lenFieldValidator(8, "The password must be at least 8 characters long")}
                  ...
                  placeholder="Your password" 
                  type="password"
                  className={`w-3/4 p-1 bg-gray-bg focus:outline-none focus:border-primary-bg border-gray-outline border-2 rounded-lg placeholder-gray-text md:w-2/5 lg:w-1/8 ${errors.repeated_password ? "border-error-light" : "border-gray-outline"} `} 
                />
                {errors.repeated_password && <div className='text-error-onBg'>{errors.repeated_password}</div>}
            <Button.Primary additionalStyle='mt-5' disabled={isSubmitting} onClick={handleSubmit}>Submit</Button.Primary>
                {errors.servererr && <div className='text-error-onBg'>{errors.servererr}</div>}
          </Form>
        }
      </Formik>
    </div>
