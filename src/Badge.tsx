...
const Badge = ({ label, link, icon, onClick, textColor }: BadgeProps) => {
  ...
  return (
    ...
    >
      ...
      <p
        className={`${textColor} underline-offset-2 underline hidden md:block truncate`}
      >
        {label}
      </p>
    </a>
...
Badge.Youtube = ({ label, link }: SubBadgeProps) => (
  <Badge
    label={label}
    link={link}
    icon={YoutubeIcon}
...
Badge.Twitter = ({ label, link }: SubBadgeProps) => (
  <Badge
    label={label}
    link={link}
    icon={TwitterIcon}
...
Badge.Instagram = ({ label, link }: SubBadgeProps) => (
  <Badge
    label={label}
    link={link}
    icon={InstagramIcon}
...
Badge.Twitch = ({ label, link }: SubBadgeProps) => (
  <Badge
    label={label}
    link={link}
    icon={TwitchIcon}
